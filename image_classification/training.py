...
def train_loop(model_and_loss, optimizer, lr_scheduler, train_loader, val_loader, debug_loader, epochs, fp16, logger,
    ...
    for epoch in epoch_iter:
        ...
        if not skip_validation:
            ...
            config.valid_history = valid_history
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0 and not skip_training:
            logger.end()
    print(valid_history)
    if skip_training:
        ...
        leverage_score(model_and_loss, optimizer, val_loader, args)
